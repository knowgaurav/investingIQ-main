...
export default function MLAnalysisView({ prediction, technical, sentiment }: MLAnalysisViewProps) {
    ...
    return (
        <div className="space-y-6">
            {/* Overall Signal Card */}
            <div className={`p-6 border-l-4 card-paper ${signal === 'BUY' ? '!border-l-gain' :
                signal === 'SELL' ? '!border-l-loss' :
                    '!border-l-accent'
                }`}>
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-display text-xl font-semibold text-theme">ML Analysis Signal</h3>
                    <span className={`px-4 py-2 font-mono text-lg font-bold tracking-widest text-white ${signal === 'BUY' ? 'bg-gain' :
                        signal === 'SELL' ? 'bg-loss' :
                            'bg-accent'
                        }`}>
                        {signal}
                ...
                {prediction ? (
                    <div className="space-y-4">
                        ...
                        {prediction.rf_signal && (
                            <div className={`rounded-lg p-4 border-2 ${prediction.rf_signal.signal === 'buy'
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className={`text-xl font-bold uppercase ${prediction.rf_signal.signal === 'buy' ? 'text-green-500' : 'text-yellow-400'
                                                }`}>
            ...
            <div className="card-paper p-6">
                ...
                {sentiment ? (
                    <div className="space-y-4">
                        {/* Sentiment Summary */}
                        <div className={`rounded-lg p-4 ${sentiment.label === 'positive' ? 'bg-green-500/10' :
                            ...
                            <div className="pt-4 border-t">
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-auto">
                                    {sentiment.details.slice(0, 8).map((detail, i) => (
                                        <div key={i} className={`flex items-start gap-3 p-2 rounded-lg ${detail.label === 'positive' ? 'bg-green-500/10' :
                                            detail.label === 'negative' ? 'bg-red-500/10' : 'bg-gray-500/20'
                                            }`}>
                                            <span className={`px-2 py-1 rounded text-xs font-bold ${detail.label === 'positive' ? 'bg-green-500 text-white' :
            ...
            <div className="bg-accent/10 border-l-4 border-accent p-4">
                <p className="text-sm text-theme-secondary">
                    <strong className="text-theme">Disclaimer:</strong> This ML analysis is for informational purposes only and should not be considered financial advice.
